...
```bash
mise run check                 # Run comprehensive checks (Rust + JS/CSS)
mise run clippy                # Lint Rust code (must pass with no warnings)
mise run fmt                   # Format Rust code
mise run lint-css              # Lint CSS only
mise run lint-web              # Lint JavaScript and CSS only
```
...
- Ensure AGENTS.md is kept up to date with the current design
